...
public class EventRegistrationPage extends JFrame {
	...
	private void refreshData() {
		...
		if(error==null||error.length()==0){
			...
			Iterator<Participant> pIt = rm.getParticipantants().iterator();
			Integer index = 0;
			while(pIt.hasNext()) {
			...
			selectedEvent = -1;
			eventList.setSelectedItem(selectedEvent);
			//participant
	...
	private void addEventButtonActionPerformed(ActionEvent evt) {
		...
		//force the same date for start and end time to ensure that only times differ
